...
"""
The greedy 'pathfinding' approach takes locally the optimal choice at each each step.
This guarantees a naieve but fast solution.
In this case, the greedy algorithm selects the closest point to the current point at each step,
starting from the depot each new string, until all points are served.
The reverse greedy approach does the same but starts at the farthest unsurved point from the depot each time.
"""
